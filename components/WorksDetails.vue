<template>
  <div
    v-if="isOpen"
    class="fixed inset-0 z-[1000] mx-auto flex max-w-[1920px] items-start justify-center overflow-y-auto overscroll-contain bg-background bg-opacity-70 p-8 md:p-20 text-copy"
    ...
    <div class="relative h-auto w-screen rounded-2xl bg-foreground" @wheel.stop>
      ...
      <div class="rounded-2xl bg-border">
        ...
        >
          ...
          >
            <h2
              class="text-center text-base font-bold sm:text-3xl"
            >
              {{ workData.title }}
            </h2>
            <span
      ...
      <div
        class="m-8 text-center max-sm:text-sm sm:p-8"
      >
        <div
          class="whitespace-pre-wrap [&>span+span]:before:mx-2 [&>span+span]:before:inline-block [&>span+span]:before:content-['/']>span+span]:before:mx-4"
